...
#
#   $SAUCELABS_APK_FILENAME     Name of the APK file that was uploaded into SauceLabs.
#                               i.e. Glass-Android-mx-v23.4-debug-RC-debug-B8.apk
#
#   $APP_PACKAGE                SauceLabs' App Package or App Identifier (review SauceLabs after uploading the app).
#                               i.e. com.walmart.mg.debug
#
...
if [ -z "${SAUCELABS_APK_FILENAME}" ]; then
    cprintError "[X] ERROR: Variable SAUCELABS_APK_FILENAME does not exist or is empty"
    exit 1
fi
cprintMsg "${G}[√] ${RESET}SAUCELABS_APK_FILENAME=${SAUCELABS_APK_FILENAME}"

# APP_PACKAGE ---------------------------------------------------------------------------------------------------------

if [ -z "${APP_PACKAGE}" ]; then
    cprintError "[X] ERROR: Variable APP_PACKAGE does not exist or is empty"
    exit 1
fi
...
if [[ $# -eq 0 ]]; then
    # DEVICE INFORMATION --------------------------------------------------------------------------------------------------
    cprintMsg "${G}[√] ${RESET}REMOTE_DEVICE_NAME_ANDROID=${REMOTE_DEVICE_NAME_ANDROID}"
    if [ -z "${REMOTE_DEVICE_NAME_ANDROID}" ]; then
        if [ -z "${DEVICE_NAME_ANDROID}" ]; then
            cprintMsg "${Y}    * ${RESET}R2 Dependencies file might define a fallback/default value for REMOTE_DEVICE_NAME_ANDROID."
        else
            cprintMsg "${Y}    * ${RESET}DEVICE_NAME_ANDROID=${DEVICE_NAME_ANDROID}. Using same value to define REMOTE_DEVICE_NAME_ANDROID."
            export REMOTE_DEVICE_NAME_ANDROID=${DEVICE_NAME_ANDROID}
        fi
    fi

    cprintMsg "${G}[√] ${RESET}REMOTE_PLATFORM_NAME_ANDROID=${REMOTE_PLATFORM_NAME_ANDROID}"
    if [ -z "${REMOTE_PLATFORM_NAME_ANDROID}" ]; then
        if [ -z "${PLATFORM_NAME_ANDROID}" ]; then
            cprintMsg "${Y}    * ${RESET}R2 Dependencies file might define a fallback/default value for REMOTE_PLATFORM_NAME_ANDROID."
        else
            cprintMsg "${Y}    * ${RESET}PLATFORM_NAME_ANDROID=${PLATFORM_NAME_ANDROID}. Using same value to define REMOTE_PLATFORM_NAME_ANDROID."
            export REMOTE_PLATFORM_NAME_ANDROID=${PLATFORM_NAME_ANDROID}
        fi
    fi

    cprintMsg "${G}[√] ${RESET}REMOTE_PLATFORM_VERSION_ANDROID=${REMOTE_PLATFORM_VERSION_ANDROID}"
    if [ -z "${REMOTE_PLATFORM_VERSION_ANDROID}" ]; then
        if [ -z "${PLATFORM_VERSION_ANDROID}" ]; then
            cprintMsg "${Y}    * ${RESET}R2 Dependencies file might define a fallback/default value for REMOTE_PLATFORM_VERSION_ANDROID."
        else
            cprintMsg "${Y}    * ${RESET}PLATFORM_VERSION_ANDROID=${PLATFORM_VERSION_ANDROID}. Using same value to define REMOTE_PLATFORM_VERSION_ANDROID."
            export REMOTE_PLATFORM_VERSION_ANDROID=${PLATFORM_VERSION_ANDROID}
        fi
    fi

    if [ -z "${DEVICE_ORIENTATION_ANDROID}" ]; then
        export DEVICE_ORIENTATION_ANDROID="portrait"
        DEVICE_ORIENTATION_ANDROID_DEFAULT_MESSAGE="   (DEFAULT - variable DEVICE_ORIENTATION_ANDROID didn't exist or was empty)"
    else
        DEVICE_ORIENTATION_ANDROID_DEFAULT_MESSAGE=""
    fi
    cprintMsg "${G}[√] ${RESET}DEVICE_ORIENTATION_ANDROID=${DEVICE_ORIENTATION_ANDROID}${DEVICE_ORIENTATION_ANDROID_DEFAULT_MESSAGE}"
    ...
    echo "No parameter received. Executing OD configurations..."
    # R2 EXECUTION --------------------------------------------------------------------------------------------------------
...
while [[ "$#" -gt 0 ]]; do
    case $1 in
        --wcp)
            ...
            if [ -z "${DEVICE_NAME_ANDROID}" ]; then
                DEVICE_NAME_ANDROID_DEFAULT_MESSAGE="${Y}    * ${RESET}R2 Dependencies file might define a fallback/default value for DEVICE_NAME_ANDROID."    
            else
                DEVICE_NAME_ANDROID_DEFAULT_MESSAGE=""
            fi
            cprintMsg "${G}[√] ${RESET}DEVICE_NAME_ANDROID=${DEVICE_NAME_ANDROID}${DEVICE_NAME_ANDROID_DEFAULT_MESSAGE}"

            if [ -z "${PLATFORM_NAME_ANDROID}" ]; then
                PLATFORM_NAME_ANDROID_DEFAULT_MESSAGE="${Y}    * ${RESET}R2 Dependencies file might define a fallback/default value for PLATFORM_NAME_ANDROID."
            else
                PLATFORM_NAME_ANDROID_DEFAULT_MESSAGE=""
            fi
            cprintMsg "${G}[√] ${RESET}PLATFORM_NAME_ANDROID=${PLATFORM_NAME_ANDROID}${PLATFORM_NAME_ANDROID_DEFAULT_MESSAGE}"

            if [ -z "${PLATFORM_VERSION_ANDROID}" ]; then
                PLATFORM_VERSION_ANDROID_DEFAULT_MESSAGE="${Y}    * ${RESET}R2 Dependencies file might define a fallback/default value for PLATFORM_VERSION_ANDROID."
            else
                PLATFORM_VERSION_ANDROID_DEFAULT_MESSAGE=""
            fi
            cprintMsg "${G}[√] ${RESET}PLATFORM_VERSION_ANDROID=${PLATFORM_VERSION_ANDROID}${PLATFORM_VERSION_ANDROID_DEFAULT_MESSAGE}"

            if [ -z "${DEVICE_ORIENTATION_ANDROID}" ]; then
                DEVICE_ORIENTATION_ANDROID_DEFAULT_MESSAGE="${Y}    * ${RESET}R2 Dependencies file might define a fallback/default value for DEVICE_ORIENTATION_ANDROID."
            else
                DEVICE_ORIENTATION_ANDROID_DEFAULT_MESSAGE=""
            fi
            cprintMsg "${G}[√] ${RESET}DEVICE_ORIENTATION_ANDROID=${DEVICE_ORIENTATION_ANDROID}${DEVICE_ORIENTATION_ANDROID_DEFAULT_MESSAGE}"
            ...
            if [ -z ${PREFIX_MAPPING_LABELS} ]; then
                ...
                exit 1
            elif [ $GLASS_ENV == preproduction ]; then
                    export MAPPING_LABELS=${PREFIX_MAPPING_LABELS}_production
                    cprintMsg s"${G}[√] ${RESET}MAPPING_LABELS=${MAPPING_LABELS}"
            else
            ...
            # R2_PROFILE ----------------------------------------------------------------------------------------------------
            if [ $GLASS_ENV == preproduction ]; then
                export R2_PROFILE=saucelabs-mx-preprod
            else
                export R2_PROFILE=saucelabs-mx-prod
            fi    
                cprintMsg "${G}[√] ${RESET}R2_PROFILE=${R2_PROFILE}"
