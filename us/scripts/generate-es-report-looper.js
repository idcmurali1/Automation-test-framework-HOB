...
const writeFileAsJsonOutput = function(testSteps, testErrors, dataJson) {
  ...
  for (let [testCaseName, testSteps] of testCases) {
    ...
    let test = {
      ...
      market: market,
      stage: fetchStage(errorTrace),
      item: fetchItem(errorTrace),
      buildId: buildId,
      branchName
    };
    tests.push(test);
...
const fetchStage = function(trace) {
  let searchTerm = 'NA';
  trace.forEach((index) => {
    const stepName = index.idName.trim();
    if (stepName.indexOf('executionStage') >= 0) {
      searchTerm = stepName.split("'")[1];
    }
  });
  return searchTerm;
};

const fetchItem = function(trace) {
  let searchTerm = 'NA';
  trace.forEach((index) => {
    const stepName = index.idName.trim();
    if (stepName.indexOf('itemFetched') >= 0) {
      searchTerm = stepName.split("'")[1];
    }
  });
  return searchTerm;
